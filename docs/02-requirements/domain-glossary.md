...
| Actor Principal        | Immutable pseudonymous UUID/kind projection provisioned through a controlled backend boundary for audit references.   | Not an identity directory, profile, credential, role, learner, or public provisioning resource.      |
...
| Term                        | Canonical definition                                                                                                    | Distinction                                                                                                 |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Language                    | Governance-owned supported human-language reference with immutable UUID and independently unique BCP 47 tag.            | ISO basis, order, capability flags, and display label do not replace identity.                              |
| Normalized Language Name    | Stored application-generated comparison value for a canonical Language label.                                           | Separate from display text; deterministic equality uses PostgreSQL `"C"` collation, not linguistic sorting. |
...
| Taxonomy Restoration        | Authorized return of `ARCHIVED` taxonomy to `ACTIVE`.                                                                   | Re-evaluates current names, ancestors, descendants, and rules; it does not restore visibility blindly.      |
...
| Taxonomy Change Record      | Restricted append-only operational audit evidence for one Category or Topic command, with exactly one typed target.     | Not learner/API content, unrestricted notes, arbitrary metadata, identity storage, or mutable history.      |
| Superseding Taxonomy Record | New immutable same-target correction linked to the terminal Taxonomy Change Record in a linear correction chain.        | Each predecessor has at most one direct successor; originals remain unchanged and branching is prohibited.  |
